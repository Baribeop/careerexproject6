...
app.get("/drugs/antibiotics", (req, res) => {
  const getAllAntibiotics = drugs.filter((each) => {
    return each.category == "Antibiotic";
  });

  res.json(getAllAntibiotics);
});

// 2.GET /drugs/names
// Return an array of all drug names converted to lowercase.
app.get("/drugs/names-in-lower-case", (req, res) => {
  const drugNameInLowerCase = drugs.map((each) => {
    return each.name.toLocaleLowerCase();
  });

  res.json(drugNameInLowerCase);
});
...
app.get("/drugs/low-stock", (req, res) => {
  const lowStockDrugs = drugs.filter((each) => {
...
app.post("/drugs/manufacturer-count", (req, res) => {
  ...
  const manufacturerDrugs = drugs.filter((each) => {
    return (each.manufacturer == manufacturer);
  });
  res.json(manufacturerDrugs.length);
});
